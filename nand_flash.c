...
const char *nand_status_str(NandStatus status)
{
    ...
    case NAND_ERR_ALREADY_PROGRAMMED:
        return "page already programmed (erase required)";
    case NAND_ERR_OUT_OF_ORDER:
        return "page programmed out of order";
    case NAND_ERR_ALLOC:
...
{
    if (!check_block_range(flash, blockIdx))
                             ...
                             const uint8_t *data, size_t dataLen)
{
    if (!check_block_range(flash, blockIdx))
        return NAND_ERR_RANGE;
    if (!check_page_range(flash, pageIdx))
        return NAND_ERR_RANGE;
...
{
    return flash->numBlocks * flash->pagesPerBlock * flash->pageSizeBytes;
}
...
size_t nand_used_bytes(const NandFlash *flash)
{
    ...
    for (size_t b = 0; b < flash->numBlocks; ++b)
    {
        for (size_t p = 0; p < flash->pagesPerBlock; ++p)
        {
            if (flash->pageStates[b][p] == PAGE_PROGRAMMED)
            {
                used += flash->pageSizeBytes;
            }
        }
    }
    return used;
